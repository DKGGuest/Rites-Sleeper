...
function App() {
  ...
  // Shared State for Batch Weighment
  ...
  if (!dutyStarted) {
    return (
      <div className="login-landing app-container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', background: 'var(--bg-color)' }}>
        <div className="section-card" style={{ textAlign: 'center', maxWidth: '400px', padding: '3rem', background: '#fff' }}>
    ...
    <div className="app-layout">
      ...
      <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
        <div className="sidebar-brand">
          <h1>SARTHI</h1>
          <p style={{ fontSize: '0.6rem', color: '#94a3b8', marginTop: '0.2rem' }}>RITES LTD.</p>
        </div>
        <nav className="sidebar-nav">
          <div className="nav-item">INSPECTION DASHBOARD</div>
          <div className="nav-item active">Sleeper Process Engineer</div>
        </nav>
        <div style={{ padding: '1rem 2rem', borderTop: '1px solid #f1f5f9', fontSize: '0.7rem', color: '#cbd5e1' }}>
          v1.1.0
        </div>
      </aside>
      ...
      <main className="main-content">
        <header className="topbar">
          <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
            <button className="close-btn" onClick={() => setSidebarOpen(true)}>☰</button>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <div className="user-avatar">I</div>
            <span style={{ fontSize: '0.85rem', color: '#64748b', cursor: 'pointer' }} onClick={() => setDutyStarted(false)}>Logout</span>
          </div>
        </header>
        ...
        <div className="app-container">
          ...
          <div className="dashboard-detail-view" style={{ background: 'none', border: 'none', boxShadow: 'none', padding: 0 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
              ...
              <button className="toggle-btn" onClick={() => setView('detail_modal')}>New Entry</button>
            </div>
            ...
            <div style={{ marginTop: '3rem' }}>
              {activeTab === 'Weight Batching' ? (
                ...
                <div style={{ textAlign: 'center', padding: '4rem 2rem', background: '#fff', borderRadius: '12px', border: '1px dotted #cbd5e1' }}>
                  ...
                  <button className="toggle-btn secondary" style={{ marginTop: '1rem' }} onClick={() => setView('detail_modal')}>Open {activeTab} Console</button>
                </div>
              )}
            </div>
          </div>
        ...
        {view === 'detail_modal' && (
          <div className="modal-container-wrapper">
            {activeTab === 'Weight Batching' ? (
              ...
              <ManualChecks onBack={() => setView('dashboard')} onAlertChange={setManualChecksAlert} />
            ) : activeTab === 'Moisture Analysis' ? (
              <MoistureAnalysis onBack={() => setView('dashboard')} onSave={() => setMoistureAlert(false)} />
              ...
              <div className="modal-overlay" onClick={() => setView('dashboard')}>
                <div className="modal-content" onClick={e => e.stopPropagation()}>
                  <header className="modal-header">
                    ...
                    <button className="close-btn" onClick={() => setView('dashboard')}>×</button>
                  </header>
